...
export default function StoryEditor({ value, onChange, dark = false, onImageUpload }) {
  ...
  const modules = useMemo(
    () => ({
      ...
      // userOnly is the important one. ReactQuill is used here as a CONTROLLED
      // component, so every time the parent re-renders it pushes the value
      // prop back into the editor via setContents with source 'api'. Quill's
      // default history (userOnly: false) records those API writes on the undo
      // stack, so Cmd+Z would revert one of them and blank the entire article
      // instead of stepping back one typed change. Limiting history to 'user'
      // changes makes undo behave the way a writer expects.
      history: {
        delay: 1000,
        maxStack: 200,
        userOnly: true,
