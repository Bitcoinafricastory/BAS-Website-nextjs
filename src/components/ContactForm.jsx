...
export default function ContactForm() {
  ...
  const handleSubmit = (e) => {
    ...
    const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=Bitcoinafricastory@proton.me&su=${encodeURIComponent(
      formData.subject
    ...
    setModal({
      ...
      message: 'A new tab is opening with your Gmail compose window. Please send the email from there!',
    });
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <input id="name" name="name" value={formData.name} onChange={handleChange} placeholder="John Doe" required
            className="w-full px-4 py-3 bg-black border border-white/10 text-white focus:outline-none focus:border-yellow-500 transition-colors duration-200" />
        </div>
        <div>
        ...
        <button type="submit" className="w-full px-6 py-3.5 sm:px-8 sm:py-4 bg-yellow-500 text-black font-medium text-base sm:text-lg hover:bg-yellow-400 transition-all duration-200 hover:scale-105 flex items-center justify-center">
