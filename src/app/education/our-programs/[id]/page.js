...
import { normalizeNonBreakingSpaces, sanitizeArticleHtml } from '@/lib/article-content';
      ...
      <div className="max-w-4xl mx-auto px-6 -mt-20 relative z-10">
        ...
        <div className="bg-[#0A0A0A] border border-white/5 rounded-2xl p-6 md:p-10 shadow-2xl">
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {program.duration && (
              <span className="flex items-center gap-1.5 px-3 py-1 bg-white/5 border border-white/10 text-gray-300 text-xs font-bold uppercase tracking-wider rounded-full">
          ...
          {program.content ? (
            <div className="article-body mx-auto mb-8" dangerouslySetInnerHTML={{ __html: sanitizeArticleHtml(normalizeNonBreakingSpaces(program.content)) }} />
          ) : program.desc ? (
          ...
          {program.link && (
            <a
              ...
              className="inline-block px-6 py-3 sm:px-8 sm:py-4 bg-yellow-500 hover:bg-yellow-400 text-black font-medium text-center rounded-lg transition-colors shadow-lg shadow-yellow-500/20"
            >
              Enrol Now
            </a>
          )}
